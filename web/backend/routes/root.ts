...
export default async function (fastify: FastifyInstance) {
  ...
  typedFastify.get('/runtimes/:pid/metrics/:serviceId', {
    ...
    const dataCpu = fastify.mappedMetrics[pid].dataCpu.filter(value => value.serviceId === serviceId)
    const dataLatency = fastify.mappedMetrics[pid].dataLatency.filter(value => value.serviceId === serviceId)
    const dataMem = fastify.mappedMetrics[pid].dataMem.filter(value => value.serviceId === serviceId)

    return { dataCpu, dataLatency, dataMem }
  })
