...
test('getMetrics respects MAX_STORED_METRICS limit', async () => {
  ...
  Object.values(fastify.mappedMetrics[1234].services).forEach(service => {
    assert.strictEqual(service.all.dataMem.length, 25, 'Service memory metrics should be limited to 25 entries')
    assert.strictEqual(service.all.dataCpu.length, 25, 'Service CPU metrics should be limited to 25 entries')
    assert.strictEqual(service.all.dataLatency.length, 25, 'Service latency metrics should be limited to 25 entries')
    assert.strictEqual(service.all.dataReq.length, 25, 'Service req metrics should be limited to 25 entries')
    assert.strictEqual(service.all.dataKafka.length, 25, 'Service kafka metrics should be limited to 25 entries')
    assert.strictEqual(service.all.dataUndici.length, 25, 'Service undici metrics should be limited to 25 entries')
    assert.strictEqual(service.all.dataWebsocket.length, 25, 'Service websocket metrics should be limited to 25 entries')

    assert.strictEqual(service[0].dataMem.length, 25, 'Worker 0 memory metrics should be limited to 25 entries')
    assert.strictEqual(service[0].dataCpu.length, 25, 'Worker 0 CPU metrics should be limited to 25 entries')
    assert.strictEqual(service[0].dataLatency.length, 25, 'Worker 0 latency metrics should be limited to 25 entries')
    assert.strictEqual(service[0].dataReq.length, 25, 'Worker 0 req metrics should be limited to 25 entries')
    assert.strictEqual(service[0].dataKafka.length, 25, 'Worker 0 kafka metrics should be limited to 25 entries')
    assert.strictEqual(service[0].dataUndici.length, 25, 'Worker 0 undici metrics should be limited to 25 entries')
    assert.strictEqual(service[0].dataWebsocket.length, 25, 'Worker 0 websocket metrics should be limited to 25 entries')

    assert.strictEqual(service[1].dataMem.length, 25, 'Worker 1 memory metrics should be limited to 25 entries')
    assert.strictEqual(service[1].dataCpu.length, 25, 'Worker 1 CPU metrics should be limited to 25 entries')
    assert.strictEqual(service[1].dataLatency.length, 25, 'Worker 1 latency metrics should be limited to 25 entries')
    assert.strictEqual(service[1].dataReq.length, 25, 'Worker 1 req metrics should be limited to 25 entries')
    assert.strictEqual(service[1].dataKafka.length, 25, 'Worker 1 kafka metrics should be limited to 25 entries')
    assert.strictEqual(service[1].dataUndici.length, 25, 'Worker 1 undici metrics should be limited to 25 entries')
    assert.strictEqual(service[1].dataWebsocket.length, 25, 'Worker 1 websocket metrics should be limited to 25 entries')
  })

  assert.strictEqual(fastify.mappedMetrics[1234].aggregated.dataMem.length, 25, 'Aggregated memory metrics should be limited to 25 entries')
  assert.strictEqual(fastify.mappedMetrics[1234].aggregated.dataCpu.length, 25, 'Aggregated CPU metrics should be limited to 25 entries')
  assert.strictEqual(fastify.mappedMetrics[1234].aggregated.dataLatency.length, 25, 'Aggregated latency metrics should be limited to 25 entries')
  assert.strictEqual(fastify.mappedMetrics[1234].aggregated.dataReq.length, 25, 'Aggregated req metrics should be limited to 25 entries')
  assert.strictEqual(fastify.mappedMetrics[1234].aggregated.dataKafka.length, 25, 'Aggregated kafka metrics should be limited to 25 entries')
  assert.strictEqual(fastify.mappedMetrics[1234].aggregated.dataUndici.length, 25, 'Aggregated undici metrics should be limited to 25 entries')
  assert.strictEqual(fastify.mappedMetrics[1234].aggregated.dataWebsocket.length, 25, 'Aggregated websocket metrics should be limited to 25 entries')
})
